...
subtitle: "COVID-19 Dominates with Steady Frequency, Other Pathogens Peak in Summer"
author: 
  - Tianrui Fu
...
Our analysis was conducted using `R` [@citeR], leveraging packages for efficient data manipulation, modeling, and visualization. Data cleaning, transformation, and visualization were facilitated by `tidyverse` [@tidyverse] and `dplyr` [@dplyr]. Poisson regression models were implemented with `rstanarm` [@rstanarm] for flexible modeling and extensions. Data integration was achieved with `arrow` [@arrow], while modeling workflows were supported using `modelr` [@modelr]. Summaries were generated with `modelsummary` [@modelsummary], and visualizations were created using `ggplot2` [@ggplot2]. Enhanced table outputs were produced with `kableExtra` [@kableExtra], dynamic reporting was handled by `knitr` [@citeknitr], file management was streamlined with `here` [@citehere], and Model API deployment was performed with `plumber` [@citeplumber].

The analysis framework followed the structured approach provided by Rohan [@citeRohan], ensuring consistency and clarity in integrating data preparation and modeling workflows. Additionally, the critique and guidelines outlined in Telling Stories With Data [@tellingstories] informed our presentation of the data and results, emphasizing clarity and coherence.

## Data Source

The dataset used in this study was obtained from Open Data Toronto [@torontooutbreaks], a publicly accessible platform providing high-quality, open government data. It was downloaded using the `opendatatoronto` [@citeopendatapkg], facilitating seamless access to the latest updates. The dataset includes 846 observations from 2024, covering 10 variables related to outbreaks in Toronto's healthcare institutions. These variables encompass key details such as outbreak settings, causative agents, dates of outbreak initiation, and other contextual factors critical for understanding infection patterns. Regular updates ensure the dataset reflects real-time trends, making it a robust resource for public health research and analysis.

## Data cleaning

The dataset was cleaned to retain variables critical to respiratory outbreaks in Toronto healthcare institutions, including `institution_address`, `outbreak_setting`, `type_of_outbreak`, `causative_agent_1`, `date_outbreak_began`, and `active`. Focus was placed on respiratory pathogens of interest: "COVID-19", "Coronavirus*", "Rhinovirus", and "Parainfluenza". A `season` variable was created by mapping months to their respective seasons, and incomplete or invalid rows were removed to ensure consistency. These steps streamlined the dataset for analysis while preserving essential details.

The count_data dataset was created by aggregating the cleaned data, grouping outbreaks by `causative_agent_1`, `season`, `month`, and `outbreak_setting`, and calculating `outbreak_count` as the total for each group. This structured summary enabled comparisons of pathogen-specific trends, seasonal patterns, and outbreak settings, forming the basis for Poisson regression models and visualizations to explore outbreak dynamics.

## Measurement
	
The data used in this study captures respiratory pathogen outbreaks in Toronto healthcare settings, collected through systematic reporting and published via Open Data Toronto [@torontooutbreaks]. Each outbreak is recorded as a discrete event, including the causative pathogen, the setting like hospital, long-term care facility, and key dates: when the outbreak began and when it was declared.
...
The @fig-count provides a more detailed and insightful view of the outcome variable `outbreak_count` by breaking it down into specific groups such as `causative_agent_1` and `season`. This grouping allows us to explore how outbreaks vary across different pathogens and seasons, providing a richer understanding of patterns within the data. The plot with just `outbreak_count` is included in [Appendix -@sec-plot-details].
...
# Create the bar plot
# Ensure the `season` variable is ordered as Spring → Summer → Fall → Winter
plot_data$season <- factor(plot_data$season, levels = c("Spring", "Summer", "Fall", "Winter"))
...
ggplot(data, aes(x = causative_agent_1, fill = causative_agent_1)) +
  geom_bar(stat = "count", position = "dodge", color = "black", alpha = 0.85) +  
  geom_text(stat = "count", aes(label = ..count..), vjust = -0.5, size = 4, fontface = "bold") + 
  scale_fill_manual(values = c(
    "COVID-19" = "olivedrab3", 
    "Coronavirus*" = "darkorange", 
    "Rhinovirus" = "#64B5F6", 
    "Parainfluenza" = "orangered" 
  )) +  
  labs(
    x = "Pathogen",
    y = "Total Count",
...
# Generate the plot
ggplot(data, aes(x = factor(month, levels = 1:11), fill = factor(month))) +
  geom_bar(color = "black", alpha = 0.9) +
  geom_text(stat = "count", aes(label = ..count..), vjust = -0.5, size = 4, fontface = "bold") +  
...
#| tbl-cap: Comparison of full and reduced model
#| warning: false


poisson_season_time <- readRDS(file = here::here("models/poisson_season_time.rds"))
reduced_model <- readRDS(file = here::here("models/reduced_selected_model.rds"))
# Compare Full Model and reduced Model

# Summaries of the models
invisible(summary(poisson_season_time))
invisible(summary(reduced_model))

# AIC and BIC comparison
aic_full <- round(AIC(poisson_season_time), 2)
bic_full <- round(BIC(poisson_season_time), 2)
aic_reduced <- round(AIC(reduced_model), 2)
bic_reduced <- round(BIC(reduced_model), 2)

# RMSE Calculation
rmse <- function(actual, predicted) {
  round(sqrt(mean((actual - predicted)^2)), 2)
}

# Predicted values
full_predictions <- predict(poisson_season_time, type = "response")
reduced_predictions <- predict(reduced_model, type = "response")

# Calculate RMSE for both models
rmse_full <- rmse(data$outbreak_count, full_predictions)
rmse_reduced <- rmse(data$outbreak_count, reduced_predictions)

# Likelihood Ratio Test
lrt <- anova(reduced_model, poisson_season_time, test = "LRT")

comparison <- data.frame(
  Metric = c("AIC", "BIC", "RMSE", "Likelihood Ratio Test (p-value)"),
  `Full Model` = c(aic_full, bic_full, rmse_full, lrt$`Pr(>Chi)`[2]),
  `Reduced Model` = c(aic_reduced, bic_reduced, rmse_reduced, "N/A") 
)
...
ggplot(data, aes(x = month, y = predicted, color = causative_agent_1)) +
  geom_rect(
    data = season_data, aes(xmin = start_month, xmax = end_month, ymin = ymin, ymax = ymax, fill = season),
