...
simulation_data <- data.frame(
  institution_address = sample(addresses, n, replace = TRUE),
  outbreak_setting = sample(outbreak_setting, n, replace = TRUE),
  type_of_outbreak = sample(type_of_outbreak, n, replace = TRUE),
  causative_agent_1 = sample(causative_agent_1, n, replace = TRUE),
  date_outbreak_began = sample(
    seq.Date(from = as.Date("2024-01-01"), to = as.Date("2024-12-31"), by = "day"), 
    n, replace = TRUE
  ),
  active = sample(c("Y", "N"), n, replace = TRUE)
)

#### Save data ####
write_csv(simulation_data, "data/00-simulated_data/simulated_data.csv")
